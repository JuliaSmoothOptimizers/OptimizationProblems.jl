...
get_channel_nvar(; n::Int = default_nvar, kwargs...) = 8 * max(2, div(n, 8))
get_channel_ncon(; n::Int = default_nvar, kwargs...) =
  4 * (max(2, div(n, 8)) - 1) + max(2, div(n, 8)) * 4 + 4
get_channel_nlin(; n::Int = default_nvar, kwargs...) = (max(2, div(n, 8)) - 1) * 4 + 4
get_channel_nnln(; n::Int = default_nvar, kwargs...) = max(2, div(n, 8)) * 4
get_channel_nequ(; n::Int = default_nvar, kwargs...) =
  4 * (max(2, div(n, 8)) - 1) + max(2, div(n, 8)) * 4 + 4
get_channel_nineq(; n::Int = default_nvar, kwargs...) = 0
