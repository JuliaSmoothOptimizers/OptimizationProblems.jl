...
function dixmaane(;
  ...
  x0 = 2 * ones(T, n)
  return ADNLPModels.ADNLPModel(f, x0, name = "dixmaan_e"; kwargs...)
end

dixmaanf(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaane(n = n, type = type, α = one(T), β = T(0.0625), γ = T(0.0625), δ = T(0.0625), name = "dixmaan_f"; kwargs...)

dixmaang(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaane(n = n, type = type, α = one(T), β = T(0.125), γ = T(0.125), δ = T(0.125), name = "dixmaan_g"; kwargs...)

dixmaanh(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaane(n = n, type = type, α = one(T), β = T(0.26), γ = T(0.26), δ = T(0.26), name = "dixmaan_h"; kwargs...)
