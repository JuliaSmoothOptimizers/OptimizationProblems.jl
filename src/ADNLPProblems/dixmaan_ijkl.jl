...
dixmaanj(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaani(n = n, type = type, α = one(T), β = T(0.0625), γ = T(0.0625), δ = T(0.0625); kwargs...)

dixmaank(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaani(n = n, type = type, α = one(T), β = T(0.125), γ = T(0.125), δ = T(0.125); kwargs...)

dixmaanl(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T} =
  dixmaani(n = n, type = type, α = one(T), β = T(0.26), γ = T(0.26), δ = T(0.26); kwargs...)
