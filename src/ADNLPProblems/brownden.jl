...
function brownden(::Val{:nlp}; m::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
...
function brownden(::Val{:nls}; m::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  m < 4 && @warn("brownden: must have m ≥ 4")
  m = max(m, 4)
  x0 = T[25.0; 5.0; -5.0; -1.0]
  t = [i // 5 for i = 1:m]
  function F!(r, x; m = m, t = t)
    tt = one(eltype(x))
    for i=1:m
      r[i] = (x[1] + t[i] * x[2] - exp(tt * t[i]))^2 + (x[3] + x[4] * sin(tt * t[i]) - cos(tt * t[i]))^2
    end
    return r
  end
