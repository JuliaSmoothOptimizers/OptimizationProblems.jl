...
function brybnd(::Val{:nlp}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
...
function brybnd(::Val{:nls}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  function F!(r, x; n = length(x))
    ml = 5
    mu = 1
    for i=1:n
      r[i] = x[i] * (2 + 5 * x[i]^2) + 1 - sum(x[j] * (1 + x[j]) for j = max(1, i - ml):min(n, i + mu) if j != i)
    end
    return r
  end
  x0 = -ones(T, n)
