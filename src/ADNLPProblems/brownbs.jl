...
function brownbs(::Val{:nlp}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  function f(x)
...
function brownbs(::Val{:nls}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  function F!(r, x)
    r[1] = x[1] - 1000000
    r[2] = x[2] - 2 * 1 // 1000000
    r[3] = x[1] * x[2] - 2
