...
function hs109(; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  ...
  function c!(cx, x)
    ...
    cx[3] = x[5] * x[6] * sin(-x[3] - 1 / 4) + x[5] * x[7] * sin(-x[4] - 1 / 4) + 2 * b * x[5]^2 -
    a * x[1] + 400 * a
    cx[4] = x[5] * x[6] * sin(x[3] - 1 / 4) + x[6] * x[7] * sin(x[3] - x[4] - 1 / 4) + 2 * b * x[6]^2 -
    a * x[2] + 400 * a
    cx[5] = x[5] * x[7] * sin(x[4] - 1 / 4) +
    x[6] * x[7] * sin(x[4] - x[3] - 1 / 4) +
    2 * b * x[7]^2 +
    881.779 * a
    cx[6] = a * x[8] + x[5] * x[6] * cos(-x[3] - 1 / 4) + x[5] * x[7] * cos(-x[4] - 1 / 4) - 200 * a -
    2 * ci * x[5]^2 + 0.7533e-3 * a * x[5]^2
    cx[7] = a * x[9] + x[5] * x[6] * cos(x[3] - 1 / 4) + x[6] * x[7] * cos(x[3] - x[4] - 1 / 4) -
    200 * a - 2 * ci * x[6]^2 + 0.7533e-3 * a * x[6]^2
    cx[8] =  x[5] * x[7] * cos(x[4] - 1 / 4) + x[6] * x[7] * cos(x[4] - x[3] - 1 / 4) + 22.938 * a -
