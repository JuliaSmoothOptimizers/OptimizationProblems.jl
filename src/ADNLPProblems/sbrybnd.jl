...
function sbrybnd(::Val{:nlp}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
...
function sbrybnd(::Val{:nls}; n::Int = default_nvar, type::Val{T} = Val(Float64), kwargs...) where {T}
  n < 2 && @warn("sbrybnd: number of variables must be ≥ 2")
  n = max(2, n)
  p = [exp(6 * (i - 1) / (n - 1)) for i = 1:n]
  J = [[max(1, i - 5):(i - 1); (i + 1):min(n, i + 1)] for i = 1:n]
