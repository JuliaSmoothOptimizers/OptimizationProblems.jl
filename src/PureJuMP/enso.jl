...
function enso(args...; kwargs...)
  ...
  @NLobjective(
    ...
    Min,
    0.5 * sum((y[i, 1] - (
      x[1] + x[2]*cos( 2*pi*y[i, 2]/12 ) + x[3]*sin( 2*pi*y[i, 2]/12 )
      + x[5]*cos( 2*pi*y[i, 2]/x[4] ) + x[6]*sin( 2*pi*y[i, 2]/x[4] )
      + x[8]*cos( 2*pi*y[i, 2]/x[7] ) + x[9]*sin( 2*pi*y[i, 2]/x[7] )
    ))^2 for i = 1:168)
  )
