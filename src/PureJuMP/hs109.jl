...
function hs109(args...; kwargs...)
  ...
  @constraint(nlp, 2250000 - x[1]^2 - x[8]^2 ≥ 0)
  @constraint(nlp, 2250000 - x[2]^2 - x[9]^2 ≥ 0)
  @NLconstraint(
    nlp,
  ...
  @NLconstraint(
    ...
    2 * c * x[5]^2 + 0.7533e-3 * a * x[5]^2 == 0
  )
  @NLconstraint(
    nlp,
    a * x[9] + x[5] * x[6] * cos(x[3] - 1 / 4) + x[6] * x[7] * cos(x[3] - x[4] - 1 / 4) - 200 * a -
    2 * c * x[6]^2 + 0.7533e-3 * a * x[6]^2 == 0
  )
  @NLconstraint(
    nlp,
    x[5] * x[7] * cos(x[4] - 1 / 4) + x[6] * x[7] * cos(x[4] - x[3] - 1 / 4) + 22.938 * a -
    2 * c * x[7]^2 + 0.7533e-3 * a * x[7]^2 == 0
  )

  @NLobjective(nlp, Min, 3 * x[1] + 1e-6 * x[1]^3 + 2 * x[2] + 0.522074e-6 * x[2]^3)
