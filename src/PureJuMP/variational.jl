...
function variational(; n::Int = default_nvar, type::Type{T} = Float64, kwargs...) where {T}
  ...
  if n == 1
    @NLexpression(model, term1, x[1] * (x[1] - 0.0) / $(h))
  else
    @NLexpression(model, term1_mid, sum(x[i] * (x[i] - x[i+1]) / $(h) for i = 1:n-1))
    @NLexpression(model, term1, term1_mid + x[n] * (x[n] - 0.0) / $(h))
  end

  # term2: build the boundary terms and interior sum separately to avoid
  # invalid indexing in the NL macro (handle n==1 specially)
  if n == 1
    @NLexpression(model, term2, (exp(x[1]) - exp(0.0)) / (x[1] - 0.0) + (exp(0.0) - exp(x[1])) / (0.0 - x[1]))
  else
    @NLexpression(model, term2_mid, sum((exp(x[j+1]) - exp(x[j])) / (x[j+1] - x[j]) for j = 1:n-1))
    @NLexpression(model, term2, (exp(x[1]) - exp(0.0)) / (x[1] - 0.0) + term2_mid + (exp(0.0) - exp(x[n])) / (0.0 - x[n]))
  end
