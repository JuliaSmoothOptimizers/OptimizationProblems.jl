...
function hs85_jump()
    ...
    @expression(m, c4,  0.04782 * (x1 - y3) + 0.1956 * (x1 - y3)^2 / x2 +
                        0.6376 * y4 + 1.594 * y3)
    ...
    @expression(m, c13, 0.995 * y10 + 60.8 * x2 + 48 * x4 -
                        0.1121 * y14 - 5095)
    ...
    # Bounds on y_i
    a = [0, 17.505, 11.275, 214.228, 7.458, 0.961, 1.612, 0.146, 107.99,
         922.693, 926.832, 18.766, 1072.163, 8961.448, 0.063, 71084.33, 2802713]

    b = [0, 1053.6667, 35.03, 665.585, 584.463, 265.916, 7.046, 0.222,
