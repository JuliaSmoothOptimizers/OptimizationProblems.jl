...
function triangle(x0 = xe, TRIS::Vector{Int64} = Tr, Const::Vector{Int64} = Constants; kwargs...) where {T}
  τ = 0.0
  n = length(x0)
  N = Int(div(n,2))
  ...
  @NLobjective(nlp, Min, sum(sum((1*x[TRIS[e+E]+N*i]-x[TRIS[e]+N*i])^2 + (2*x[TRIS[e+2*E]+N*i]-x[TRIS[e+E]+N*i]-x[TRIS[e]+N*i])^2 / 3 for i =0:1)/(2*(2*((x[TRIS[e+E]]-x[TRIS[e]])*(x[TRIS[e+2*E]+N]-x[TRIS[e]+N])-(x[TRIS[e+2*E]]-x[TRIS[e]])*(x[TRIS[e+E]+N]-x[TRIS[e]+N])) / sqrt(3))) for e=1:E))

  for e = 1:E
    @NLconstraint(nlp, 2*((x[TRIS[e+E]]-x[TRIS[e]])*(x[TRIS[e+2*E]+N]-x[TRIS[e]+N])-(x[TRIS[e+2*E]]-x[TRIS[e]])*(x[TRIS[e+E]+N]-x[TRIS[e]+N])) / sqrt(3) >= τ)
  end
