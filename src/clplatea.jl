...
function clplatea(n::Int=5041, wght::Float64=-0.1)
  ...
  @NLobjective(
    nlp,
    Min,
    (wght * x[p,p]) + sum(sum(0.5 * (x[i,j] - x[i,j-1])^2 + hp2 * (x[i,j] - x[i,j-1])^4 for j=2:p) for i=2:p) + sum(0.5 * (x[2,j])^2 +	hp2 * (x[2,j])^4 for j=2:p) + sum(sum(0.5 * (x[i,j] - x[i-1,j])^2 +	hp2 * (x[i,j] - x[i-1,j])^4 for j=2:p) for i=3:p)
  )
